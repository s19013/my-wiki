...
test.describe("画面遷移",() => {
    ...
    // 条件:初めての方はこちらボタンを押す
    // 期待:新規登録画面が表示される
    test('初めての方はこちらボタンを押す', async ({ page }) => {
        await page.goto(baseHome);
    ...
    // 条件:初めての方はこちらリンクを押す
    // 期待:新規登録画面が表示される
    test('初めての方はこちらリンクを押す', async ({ page }) => {
        await page.goto(baseHome);
        await page.getByRole('button', { name: '初めての方はこちら' }).click();
        await expect(page).toHaveURL(baseHome + "register");
    })
