<template>
    <BaseLayout :title="`${articleTitle} ${title}`" :pageTitle="pageTitle">
        <div class="articleContainer">
            ...
            <DateLabel
                v-if="edit"
                :createdAt="originalArticle.created_at"
                :updatedAt="originalArticle.updated_at"
            />
            ...
            <p
                ...
                v-for="message of errorMessages.others"
                :key="message"
            ...
            <v-form v-on:submit.prevent>
                ...
                <p
                    ...
                    v-for="message of errorMessages.articleTitle"
                    :key="message"
                    class="global_css_error"
                >
                    <v-icon>mdi-alert-circle-outline</v-icon>
        ...
        <loadingDialog />
        <v-snackbar v-model="successed" :timeout="1000">
            {{ messages.success }}
        </v-snackbar>
    </BaseLayout>
</template>
...
export default {
    data() {
        return {
            ...
            messages: {
                ...
                title: "title",
                otherError:
                    "An error occurred on the server side, please wait a few seconds and try again",
                success: "saved",
            },
    ...
    methods: {
        ...
        focusToBody() {
            this.$refs.articleBody.focusToBody();
        },
        changeTab() {
            this.$refs.articleBody.changeTab();
        },
        ...
        setErrors(errors) {
            ...
            if (String(errors.status)[0] == 5) {
                ...
                this.errorMessages = errors.data.messages;
            }
        },
        keyEvents(event) {
